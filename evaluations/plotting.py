...
def plot_acb(task, args):
    ...
    paths = []
    for i in range(n_trials):
        p = os.path.join(results_dir, "{}.cumulative_reward.{}.csv".format(task, i))
        paths.append(p)
    M_cr_mean, M_cr_std, columns = compute_mean_std(paths, n_trials)
    plot_cumulative_reward(task, M_cr_mean, M_cr_std, columns)
...
def plot_cb(task, args):
    ...
    # Get the path to the results of all trials.
    paths = []
    for i in range(n_trials):
        p = os.path.join(results_dir, "{}.cumulative_regret.{}.csv".format(task, i))
        paths.append(p)

    M_cr_mean, M_cr_std, columns = compute_mean_std(paths, n_trials)

    plot_cumulative_regret(task, M_cr_mean, M_cr_std, columns)

    paths = []
    for i in range(n_trials):
        p = os.path.join(results_dir, "{}.actions.{}.csv".format(task, i))
        paths.append(p)
    # @todo: fix this
    paths = [paths[0]]

    plot_actions(task, paths, columns)


def plot_cumulative_regret(task_name, M_cr_mean, M_cr_std, columns):
    high, low = M_cr_std
    ...
    xs = np.arange(M_cr_mean.shape[0])
    for j in range(M_cr_mean.shape[1]):
        y_mean = M_cr_mean[:, j]

        ax.plot(xs, y_mean + j * 0.01, label=columns[j], linewidth=1)

        ax.fill_between(xs, np.maximum(0, low[:, j]), high[:, j], alpha=0.1)

    ax.legend(loc="upper left", fontsize=15)
    fig.savefig("{}/{}.cumulative_regret.pdf".format(results_dir, task_name), bbox_inches="tight")
    plt.close()


def plot_actions(task_name, paths, columns):
    ...
    for p in paths:
        ...
        for j in range(M_acts.shape[1]):
            y = M_acts[:, j]
            plt.scatter(np.arange(M_acts.shape[0]), y + j * 0.05, label=columns[j], s=1)
        plt.legend(bbox_to_anchor=(1.04, 1), borderaxespad=0, markerscale=6, fontsize=15)

    plt.savefig("{}/{}.actions.pdf".format(results_dir, task_name), bbox_inches="tight")

    plt.close()


def plot_cumulative_reward(task_name, M_cr_mean, M_cr_std, columns):
    high, low = M_cr_std
    fig, ax = plt.subplots(figsize=(7, 7))
    ...
    ax.legend(loc="upper left", fontsize=15)
    fig.savefig("{}/{}.cumulative_reward.pdf".format(results_dir, task_name), bbox_inches="tight")
    plt.close()


def plot_CTR(task_name, M_cr_mean, M_cr_std, columns, window):
    # Plots the click-through rate (number of clicks divided by number of visits)
    high, low = M_cr_std
    fig, ax = plt.subplots(figsize=(7, 7))
    ax.set_ylim([0.0, 0.4])
    ...
    ax.legend(loc="upper left", fontsize=15)
    fig.savefig("{}/{}.CTR.pdf".format(results_dir, task_name), bbox_inches="tight")
    plt.close()
